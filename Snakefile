...
rule prep_nsdsyn_data:
    input:
        ...
        lh_prfs = expand(os.path.join(config['NSD_DIR'], 'nsddata', 'freesurfer','{{subj}}', 'label', 'lh.prf{prf_param}.mgz'), prf_param= ["eccentricity", "angle", "size", "exponent", "R2"]),
        ...
        rh_prfs= expand(os.path.join(config['NSD_DIR'],'nsddata','freesurfer','{{subj}}','label','rh.prf{prf_param}.mgz'), prf_param=["eccentricity", "angle", "size", "exponent", "R2"]),
...
rule plot_tuning_curves_NSD:
    ...
    run:
        ...
        vis1D.plot_tuning_curves_NSD(data_df=bin_df, subj=wildcards.subj,
...
rule shuffle_local_sf:
    """Shuffle local_sf across stimulus classes for spatial frequency null test."""
    input:
        subj_df = os.path.join(config['OUTPUT_DIR'], 'dataframes', '{dset}', 'model',
                               'dset-{dset}_sub-{subj}_roi-{roi}_vs-{vs}_tavg-False.csv'),
        precision = os.path.join(config['OUTPUT_DIR'], 'dataframes', '{dset}', 'precision',
    ...
    log:
        os.path.join(config['OUTPUT_DIR'], 'logs', 'dataframes', '{dset}', 'perm',
                     ...
                     'perm-{perm}_dset-{dset}_sub-{subj}_roi-{roi}_vs-{vs}_shuffle.log')
    run:
        from sfp_nsdsyn.bootstrapping import shuffle_local_sf
        np.random.seed(int(wildcards.perm))
        subj_df = pd.read_csv(input.subj_df)
        precision_df = pd.read_csv(input.precision)
        df = subj_df.merge(precision_df, on=['sub', 'vroinames', 'voxel'])
...
rule plot_null_param_distributions:
    ...
    input:
        ...
        null_ori_models = lambda wc: [
            perm_model_pt('nsdsyn', subj, p, wc.lr, wc.max_epoch, 'V1', wc.vs, shuffle_type='orientation')
            for p in range(int(wc.n_perm))
            for subj in make_subj_list('nsdsyn')],
        null_sf_models = lambda wc: [
            perm_model_pt('nsdsyn', subj, p, wc.lr, wc.max_epoch, 'V1', wc.vs, shuffle_type='local_sf')
            for p in range(int(wc.n_perm))
            for subj in make_subj_list('nsdsyn')]
    output:
    ...
    run:
        ...
        ecc_params = ['slope', 'intercept']
        null_ecc = model.load_all_models(input.null_ecc_models, *meta_args, params=ecc_params)

        ori_params = ['p_1', 'p_2', 'p_3', 'p_4', 'A_1', 'A_2']
        null_ori = model.load_all_models(input.null_ori_models, *meta_args, params=ori_params)

        sf_params = ['sigma']
        null_sf = model.load_all_models(input.null_sf_models, *meta_args, params=sf_params)

        # Merge on [sub, perm]
        null_ecc['perm'] = null_ecc['perm'].astype(int)
        null_ori['perm'] = null_ori['perm'].astype(int)
        null_sf['perm'] = null_sf['perm'].astype(int)
        null_nsd_df = null_ecc.merge(null_ori, on=meta_cols).merge(null_sf, on=meta_cols)

        # Mean across subjects per permutation
        null_param_df = null_nsd_df.groupby('perm')[PARAMS_2D].mean().reset_index()

        # Transform sigma -> 1/sigma
...
rule plot_combined_error_mse_comparison:
    ...
    Uses three shuffle types for per-parameter null distributions:
      - shuffle_eccentricity -> slope, intercept
      - shuffle_orientation  -> p_1, p_2, p_3, p_4, A_1, A_2
      - shuffle_local_sf     -> sigma
    Per-parameter errors are standardized or raw based on {standardize} wildcard.
    MSE, correlation, and scatter plots include all params (including sigma).
    """
    input:
        nsd_models = expand(os.path.join(config['OUTPUT_DIR'], "sfp_model", "results_2D", "nsdsyn",
                            'model-params_lr-{{lr}}_eph-{{max_epoch}}_sub-{subj}_roi-V1_vs-{{vs}}.pt'),
    ...
    run:
        ...
        # Load null datasets efficiently - only extract needed params per shuffle type
        ecc_params = ['slope', 'intercept']
        ori_params = ['p_1', 'p_2', 'p_3', 'p_4', 'A_1', 'A_2']
        sf_params = ['sigma']
        meta_cols = ['sub', 'perm']
        ...
        null_ecc_df = model.load_all_models(input.null_ecc_models, *meta_args, params=ecc_params)
        null_ecc_df['perm'] = null_ecc_df['perm'].astype(int)

        null_ori_df = model.load_all_models(input.null_ori_models, *meta_args, params=ori_params)
        null_ori_df['perm'] = null_ori_df['perm'].astype(int)

        null_sf_df = model.load_all_models(input.null_sf_models, *meta_args, params=sf_params)
        null_sf_df['perm'] = null_sf_df['perm'].astype(int)
        ...
        pooled_sd_df = pooled_std(combined, group_col='dset_type', params=params_scatter)
        std_means = standardized_mean(combined, pooled_sd_df, group_col='dset_type', params=params_scatter)
...
rule  fit_to_bootstraps:
    input:
        expand(os.path.join(config['OUTPUT_DIR'], "sfp_model","results_2D", "{dset}", 'bootstraps', 'bootstrap-{bts}_model-params_lr-{lr}_eph-{max_epoch}_sub-{subj}_roi-{roi}_vs-{vs}.pt'),
               bts=np.arange(0,1000), lr=LR_2D, max_epoch=MAX_EPOCH_2D, dset='nsdsyn', subj=make_subj_list('nsdsyn'), roi=ROIS, vs='pRFsize')
...
rule save_subj_cov_matrix:
    ...
    run:
        ...
        cov_matrix = sim.measure_noise_covariance(subj_data, 'betas', 
...
rule debug_simulation:
    ...
    benchmark:
        os.path.join(config['OUTPUT_DIR'], "benchmark", "dataframes", "simulation", "debug_roi-{roi}_grating-{grating_type}_cov-True_noise-{noise_lvl}_basesub-{sub}_slope-{slope}_rnseed-{seed}.txt")
    log:
        os.path.join(config['OUTPUT_DIR'], "logs", "dataframes", "simulation", "debug_roi-{roi}_grating-{grating_type}_cov-True_noise-{noise_lvl}_basesub-{sub}_slope-{slope}_rnseed-{seed}.log")
    run:
        subj_data = pd.read_csv(input.subj_df_path)
        subj_data['trial'] = subj_data.groupby(['sub','voxel','class_idx']).cumcount()
        ...
        params_dict = params_info.to_dict(orient='records')[0]
        print(params_dict)
        params_info.to_csv(output[0], index=False)
        # syn = sim.SynthesizeData(roi=wildcards.roi, 
        #                          df=subj_data,
        #                          random_state=wildcards.seed)                                 
        # syn_data = syn.synthesize_BOLD_2d(params_dict,
...
rule run_simulation:
    ...
    run:
        ...
        my_dataset = model.SpatialFrequencyDataset(syn_data, beta_col='noisy_betas')
